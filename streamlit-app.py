...
def update_data():
    ...
    st.markdown("""---""")
    st.header("Top Polling Stations (Votes Cast by Polling Station)")
    top_stations_consumer = create_kafka_consumer("top_polling_stations")
    top_stations_data = fetch_data_from_kafka(top_stations_consumer)
    top_polling_df = pd.DataFrame(top_stations_data)

    if not top_polling_df.empty and {"polling_station", "votes_cast"}.issubset(top_polling_df.columns):
        top_polling_df = top_polling_df.sort_values("votes_cast", ascending=True)
        fig = px.bar(
            top_polling_df,
            x="votes_cast",
            y="polling_station",
            orientation="h",
            title="Top Polling Stations by Votes Cast",
            labels={"votes_cast": "Votes Cast", "polling_station": "Polling Station"},
            height=600
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Polling station data not available or schema mismatch.")

    # ✅ Voters by Education & Party Affiliation
    st.markdown("""---""")
    st.header("Voters by Education Level and Party Affiliation")
    edu_party_consumer = create_kafka_consumer("voters_by_education_party")
    edu_party_data = fetch_data_from_kafka(edu_party_consumer)
    edu_party_df = pd.DataFrame(edu_party_data)

    if not edu_party_df.empty and {"education_level", "party_affiliation", "num_voters"}.issubset(edu_party_df.columns):
        fig = px.bar(
            edu_party_df,
            x="education_level",
            y="num_voters",
            ...
            barmode="group",
            title="Voter Distribution by Education and Party Affiliation",
            labels={"education_level": "Education Level", "num_voters": "Number of Voters"},
            height=600
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Education/party data not available or schema mismatch.")

    # ✅ Hourly Turnout Line Chart
    st.markdown("""---""")
    st.header("Hourly Votes")
    hourly_turnout_consumer = create_kafka_consumer("turnout_by_hour")
    hourly_turnout_data = fetch_data_from_kafka(hourly_turnout_consumer)
    hourly_df = pd.DataFrame(hourly_turnout_data)

    if not hourly_df.empty and {"hour_of_day", "total_votes"}.issubset(hourly_df.columns):
        ...
        fig = px.line(
            ...
            y="total_votes",
            markers=True,
            title="Votes Cast by Hour of Day",
            labels={"hour_of_day": "Hour of Day", "total_votes": "Total Votes"},
            height=500
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Hourly turnout data not available or schema mismatch.")
